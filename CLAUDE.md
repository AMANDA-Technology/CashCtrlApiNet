...
3. **`HttpClient` is created directly in `CashCtrlConnectionHandler`** -- not using `IHttpClientFactory`. This is a limitation for DI/testability.
4. **POST uses form-encoded content** -- The CashCtrl API expects `application/x-www-form-urlencoded` for writes, not JSON.
5. **Language query parameter has a trailing space** -- In `CashCtrlConnectionHandler.GetHttpRequestMessage`, the language key is `"lang "` (with a space). This appears to be a bug.
