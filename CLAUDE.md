...
8. **DI registration uses `IHttpClientFactory`** -- `AddCashCtrl` registers `CashCtrlConnectionHandler` via `AddHttpClient<>`, ensuring proper `HttpClient` lifetime management through the factory pattern.
